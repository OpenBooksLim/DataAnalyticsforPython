...
url = 'http://ws.bus.go.kr/api/rest/buspos/getBusPosByRtid' # 노선 버스 위치정보 목록조회(busRouteId)

busRouteId = ['100100118']
c = []
for i in range(0, len(busRouteId)):
    params = 'serviceKey=%s&busRouteId=%s' % (cm.get_serviceKey(), busRouteId[i])
    print('params : ', params)

    # 저장을 원할 경우, 화일명을 정의한다.
    fn = os.getcwd() + '/data/' + '3-2_%s.da' % str(busRouteId[i])
    # 서비스 호출
...
# stopFlag : 정류소도착여부(0:운행중, 1:도착), busType : 차량유형(0:일반버스, 1:저상버스, 2:굴절버스)
